...
| `GET` | `/training` | Training dashboard (HTML) |

### Health

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/health` | Returns `{"status": "alive", "version": "2.0.0"}` |

### Chat

| Method | Path | Body | Response |
|--------|------|------|----------|
| `POST` | `/api/chat` | `{"text": "question"}` | `{"response": "...", "confidence": 0.8, "concepts": [...], "duration_ms": 150}` |

### Knowledge

| Method | Path | Body | Response |
|--------|------|------|----------|
| `POST` | `/api/learn` | `{"concept": "banana"}` | `{"concept": "banana", "associations": 20, "volatility": 1.0}` |
...
|--------|------|----------|
| `GET` | `/api/brain/status` | Full brain state snapshot (memory, training, agents, concepts) |
...
| Method | Path | Response |
|--------|------|----------|
| `GET` | `/api/training/status` | Training engine status (running, cycle, uptime, latest_metrics) |
...
| Method | Path | Response |
|--------|------|----------|
| `GET` | `/api/agents/status` | Agent manager status (cycle, online, gap_queue_size) |
...
```json
{
  ...
  "training": {"running": true, "cycle": 42, ...},
  "agents": {"cycle": 15, "online": true, ...},
  "chat_history": [...],
  "recent_concepts": [{"concept": "banana", "volatility": 0.1, ...}]
}
```
